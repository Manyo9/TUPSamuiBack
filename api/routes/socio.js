...
router.get('/reportePuntos/:limite', [authJwt.verifyToken, authJwt.invalidTokenCheck, authJwt.isEmployee], (req, res) => {

    mysqlConnecction.query('call spSociosConMasPuntos(?)', [req.params['limite']],
        (err, rows, fields) => {
            if (!err) {
                res.status(200).json({
                    "ok": true,
                ...
                res.status(500).json({
                    "ok": false,
                    "mensaje": "Error al generar listado de socios"
                });
            }
        });
